...
pub(crate) struct ClaudeCodeAuthMaterial {
    ...
    pub(crate) static_auth_token: bool,
}
...
pub(crate) fn claudecode_access_token_from_credential(
    ...
    let explicit_access_token = credential.access_token.trim().to_string();
    ...
    let static_auth_token = auth_token.is_some() && refresh_token.is_empty() && cookie.is_none();
    let access_token = if explicit_access_token.is_empty() {
...
pub(crate) async fn resolve_claudecode_access_token(
    ...
    let _ = cache_key;
    if material.static_auth_token && !material.access_token.trim().is_empty() {
...
pub(crate) async fn refresh_claudecode_access_token(
    ...
    if material.static_auth_token && !material.access_token.trim().is_empty() {
        return Ok(ClaudeCodeRefreshedToken {
            ...
            subscription_type: material.subscription_type.clone(),
            rate_limit_tier: material.rate_limit_tier.clone(),
